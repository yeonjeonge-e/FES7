...
class VendingMachineFunc {
  ...
  constructor() {
    ...
    // balance : 잔액
    // this : 인스턴스 가리킴
    // this 사용이유 ? --- 클래스로 생성한 인스턴스 가리킴
    // this 대신 변수선언을 하면 balance 만 출력됨
    this.itemList = vMachine.querySelector('.cola-list');
    this.inputCostEl = vMachine.querySelector('#input-money');    //  인풋창
    this.btnPut = vMachine.querySelector('#input-money+.btn');    //  입금
    this.btnReturn = vMachine.querySelector('.bg-box+.btn');
    this.btnGet = vMachine.querySelector('.btn-get');
    this.stageList = vMachine.querySelector('.get-list');

    // section 2 요소 선택
    const myInfo = document.querySelector('.section2');
    this.myMoney = myInfo.querySelector('.bg-box p');             // 소지금
...
stagedItemGenerator(target) {
  ...
  `;
  this.stageList.append(stagedItem);
}
  ...
  bindEvents() {
     ...
     *    소지금 === 소지금 - 입금액
     *    잔액 === 기존액 잔액 + 입금액
     *    입금액이 소지금보다 많으면 "소지금이 부족합니다" 라는 경고창 띄우기
     *    입금액 인풋창은 초기화 되어야 함
     */
    this.btnPut.addEventListener('click', () => {
      ...
      // 사용자가 입력한 값이 없으면 실행이 되면 안됨
      // 입금액이 있고, 소지금 보다 적거나 같다면
      if (inputCost && inputCost > 0) {
        if (inputCost <= myMoneyVal) {

          // Intl 객체를 통해서 사용자 국가에서 사용하는 포멧으로 숫자를 변경함
          this.myMoney.textContent = new Intl.NumberFormat().format(myMoneyVal - inputCost) + ' 원';
          // this.myMoney 안에 있는 금액이 언제 바뀔 지 몰라서 적어주는 것
          // textContent : 현재 myMoney의 잔액
          // parseInt 붙일 필요 없음 - 그냥 대상을 선택한거라
          // Intl (객체) : Intl.NumberFormat() 안쓰면 , 표시가 안붙음

          this.balance.textContent = new Intl.NumberFormat().format((balanceVal ? balanceVal : 0) + inputCost) + ' 원';
          // format() 안에서는 연산식만 쓸 수 있음
          // (balanceVal ? balanceVal : 0) 

        } else {
          alert("소지금이 부족합니다");
        }

        // 사용자가 입력한 입금액 초기화 하는 부분
     ...
     *    만약 콜라의 data-count 값이 0이라면 button 요소에 disabled 속성이 추가되고,
     *      콜라 템플릿에 strong 태그가 추가되어야 합니다.
     */
    ...
    colaBtns.forEach((colaBtn) => {
        ...
        // [10번] 코드 추가
        const stagedListItem = this.stageList.querySelectorAll('li');
    ...
    this.btnGet.addEventListener('click', () => {
      const itemStagedList = this.stagedList.querySelectorAll('il');
      const itemGetList = this.getList.querySelectorAll('il');
      let totalPrice = 0;

      // 장바구니 아에템을 획득 목록으로 이동하기
